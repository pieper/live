...
// render/colorize-field.ts
var ColorizeField = class {
  kind = "clz";
  bindingCount = 3;
  // ct (3d f32) + labels (3d u32) + luts (2d)
  clippable;
  dev;
  ctTex;
  labTex;
  lutTex;
  // 256x2: row 0 = CT transfer function, row 1 = palette
  p2t;
  box;
  stepMm;
  dims;
  ...
  constructor(dev, ct, labels, dims, ctLut, opts) {
    this.dev = dev;
    this.dims = dims;
    const size = dims;
    ...
    this.clim = opts.clim;
    this.shade = opts.shade ?? [0.25, 0.8, 0.3, 18];
  ...
  setCT(ct) {
    this.dev.queue.writeTexture(
      { texture: this.ctTex },
      toF16Array(ct),
      { bytesPerRow: this.dims[0] * 2, rowsPerImage: this.dims[1] },
      this.dims
    );
    this.hasCt = true;
  }
  ...
  setSegmentOpacity(num, a) {
    this.palette[num * 4 + 3] = Math.round(Math.max(0, Math.min(1, a)) * 255);
  }
  getSegmentOpacity(num) {
    return this.palette[num * 4 + 3] / 255;
  }
  ...
  // mat4(16) + clim(4) + shade(4) + params(4) + dims(4)
  aabb() {
    return this.box;
  }
  sampleStep() {
  ...
  structMembers(s) {
    return [
      ...
      `  clz${s}_clim : vec4<f32>,`,
      // lo, hi, _, _
      `  clz${s}_shade : vec4<f32>,`,
      // ka, kd, ks, shininess
      `  clz${s}_params : vec4<f32>,`,
      // opacity_unit_distance, contextOpacity, ctModulation, _
      `  clz${s}_dims : vec4<f32>,`
      // nx, ny, nz, _
    ].join("\n");
  }
...
fn sample_field_clz${s}(wp : vec3<f32>, rd : vec3<f32>) -> vec4<f32> {
  ...
  fillUniforms(out, off2) {
    ...
    out[off2 + 25] = this.context;
    out[off2 + 26] = this.ctMod;
    out[off2 + 28] = this.dims[0];
    out[off2 + 29] = this.dims[1];
    out[off2 + 30] = this.dims[2];
  }
  bindEntries(_s, base) {
    return [
...
// examples/colorize/ct-presets.ts
var CT_PRESETS = {
  "CT-Soft-Tissue": {
    color: [[-2048, 0, 0, 0], [-167.01, 0, 0, 0], [-160, 0.0556, 0.0556, 0.0556], [240, 1, 1, 1], [3661, 1, 1, 1]],
    opacity: [[-2048, 0], [-167.01, 0], [-160, 1], [240, 1], [3661, 1]],
    shade: [0.2, 1, 0, 1],
    windowLevel: [400, 40]
  },
  "CT-Muscle": {
    color: [[-3024, 0, 0, 0], [-155.407, 0.549, 0.251, 0.149], [217.641, 0.8824, 0.6039, 0.2902], [419.736, 1, 0.937, 0.9545], [3071, 0.8275, 0.6588, 1]],
    opacity: [[-3024, 0], [-155.407, 0], [217.641, 0.6765], [419.736, 0.8333], [3071, 0.8039]],
    shade: [0.1, 0.9, 0.2, 10],
    windowLevel: [400, 40]
  },
  "CT-Bone": {
    color: [[-3024, 0, 0, 0], [-16.4458, 0.7294, 0.2549, 0.302], [641.385, 0.9059, 0.8157, 0.5529], [3071, 1, 1, 1]],
    opacity: [[-3024, 0], [-16.4458, 0], [641.385, 0.7157], [3071, 0.7059]],
    shade: [0.1, 0.9, 0.2, 10],
    windowLevel: [1800, 400]
  },
  "CT-Air": {
    color: [[-3024, 1, 1, 1], [-900, 0.2, 1, 1], [-500, 0.3, 0.3, 1], [3071, 0, 0, 0]],
    opacity: [[-3024, 0.7059], [-900, 0.7157], [-500, 0], [3071, 0]],
    shade: [0.1, 0.9, 0.2, 10],
    windowLevel: [1400, -500]
  },
  "CT-Lung": {
    color: [[-1e3, 0.3, 0.3, 1], [-600, 0, 0, 1], [-530, 0.1347, 0.7817, 0.0725], [-460, 0.9292, 1, 0.1095], [-400, 0.8889, 0.2549, 0.024], [2952, 1, 0.3, 0.3]],
    opacity: [[-1e3, 0], [-600, 0], [-599, 0.15], [-400, 0.15], [-399, 0], [2952, 0]],
    shade: [0.2, 1, 0, 1],
    windowLevel: [1500, -600]
  },
  "CT-Fat": {
    color: [[-1e3, 0.3, 0.3, 1], [-497.5, 0.3, 1, 0.3], [-99, 0, 0, 1], [-76.946, 0, 1, 0], [-65.481, 0.8354, 0.8889, 0.0165], [83.89, 1, 0, 0], [463.28, 1, 0, 0], [659.15, 1, 0.9125, 0.0375], [2952, 1, 0.3003, 0.2999]],
    opacity: [[-1e3, 0], [-100, 0], [-99, 0.15], [-60, 0.15], [-59, 0], [101.2, 0], [952, 0]],
    shade: [0.2, 1, 0, 1],
    windowLevel: [400, -60]
  },
  "CT-AAA": {
    color: [[-3024, 0, 0, 0], [143.556, 0.6157, 0.3569, 0.1843], [166.222, 0.8824, 0.6039, 0.2902], [214.389, 1, 1, 1], [419.736, 1, 0.937, 0.9545], [3071, 0.8275, 0.6588, 1]],
    opacity: [[-3024, 0], [143.556, 0], [166.222, 0.6863], [214.389, 0.6961], [419.736, 0.8333], [3071, 0.8039]],
    shade: [0.1, 0.9, 0.2, 10],
    windowLevel: [700, 150]
  },
  "CT-Cardiac3": {
    color: [[-3024, 0, 0, 0], [-86.9767, 0, 0.251, 1], [45.3791, 1, 0, 0], [139.919, 1, 0.8949, 0.8949], [347.907, 1, 1, 0.251], [1224.16, 1, 1, 1], [3071, 0.8275, 0.6588, 1]],
    opacity: [[-3024, 0], [-86.9767, 0], [45.3791, 0.1696], [139.919, 0.5893], [347.907, 0.6071], [1224.16, 0.6071], [3071, 0.6161]],
    shade: [0.1, 0.9, 0.2, 10],
    windowLevel: [1400, 300]
  },
  "CT-Chest-Contrast-Enhanced": {
    color: [[-3024, 0, 0, 0], [67.0106, 0.549, 0.251, 0.149], [251.105, 0.8824, 0.6039, 0.2902], [439.291, 1, 0.937, 0.9545], [3071, 0.8275, 0.6588, 1]],
    opacity: [[-3024, 0], [67.0106, 0], [251.105, 0.4464], [439.291, 0.625], [3071, 0.6161]],
    shade: [0.1, 0.9, 0.2, 10],
    windowLevel: [1400, 300]
  },
  "CT-Chest-Vessels": {
    color: [[-3024, 0, 0, 0], [-1278.35, 0.549, 0.251, 0.149], [22.8277, 0.8824, 0.6039, 0.2902], [439.291, 1, 0.937, 0.9545], [3071, 0.8275, 0.6588, 1]],
    opacity: [[-3024, 0], [-1278.35, 0], [22.8277, 0.4286], [439.291, 0.625], [3071, 0.6161]],
    shade: [0.2, 1, 0, 1],
    windowLevel: [1400, 300]
  },
  "CT-Pulmonary-Arteries": {
    color: [[-2048, 0, 0, 0], [-568.625, 0, 0, 0], [-364.081, 0.3961, 0.302, 0.1804], [-244.813, 0.6118, 0.3529, 0.0706], [18.2775, 0.8431, 0.0157, 0.1569], [447.798, 0.7529, 0.7529, 0.7529], [3592.73, 1, 1, 1]],
    opacity: [[-2048, 0], [-568.625, 0], [-364.081, 0.0714], [-244.813, 0.4018], [18.2775, 0.6071], [447.798, 0.8304], [3592.73, 0.8393]],
    shade: [0.2, 1, 0, 1],
    windowLevel: [1e3, 100]
  },
  "CT-X-ray": {
    color: [[-3024, 1, 1, 1], [3071, 1, 1, 1]],
    opacity: [[-3024, 0], [-200, 0], [1500, 0.05], [3071, 0.05]],
    shade: [0.1, 0.9, 0.2, 10],
    windowLevel: [2e3, 300]
  }
};
var CT_PRESET_NAMES = Object.keys(CT_PRESETS);

// examples/colorize/colorize-scene.ts
var GROUP_DEFAULT_OPACITY = {
  Organs: 0.9,
  Heart: 0.9,
  Vessels: 0.9,
  Intestines: 0.55,
...
async function buildColorizeScene(gpu2, base, format, onProgress2) {
  ...
  const segments = manifest.segments.map((s) => ({
    num: s.num,
    name: s.name,
    color: [s.color[0] / 255, s.color[1] / 255, s.color[2] / 255]
  }));
  for (const s of segments) field.setSegmentColor(s.num, s.color);
  let onCtArrived = null;
  ...
  let roi = createRoiWidget(lo, hi, { coverage: 0.45 });
  ...
  const out = {
    ...
    ctReady,
    ctLoaded: () => field.ctLoaded,
    contextOpacity: () => field.getContextOpacity(),
    setContextOpacity(o) {
      field.setContextOpacity(o);
      scene.syncUniforms();
    },
    cropEnabled: () => cropOn,
    roiVisible: () => roiOn,
  ...
  if (controls.length) {
    for (const c of controls) {
      ...
      if (c.slider) {
        ...
        const fmt2 = c.slider.format ?? ((v) => String(Math.round(v)));
        const paint = () => {
          val.textContent = fmt2(c.slider.get());
        };
        inp.oninput = () => {
...
// examples/colorize/colorize-browser.ts
...
var DATA_BASE = new URLSearchParams(location.search).get("data") ?? "https://js2.jetstream-cloud.org:8001/swift/v1/slicerlive/colorize/";
var barFill = $("barfill");
var loadPct = $("loadpct");
...
camera.position = [sc.center[0], sc.center[1] + sc.radius * 2.4, sc.center[2]];
camera.viewUp = [0, 0, 1];
camera.viewAngle = 30;
var a3d = mountAdaptive3d({
  ...
  gpu,
  movingScaleCap: 0.7,
  // 55 M voxels with two texture fetches per sample
  onFrame: () => cross2.redraw()
});
var draw3d = () => a3d.draw();
...
var labelAt = (ras) => {
  ...
  const t = [ras[0] - m[3], ras[1] - m[7], ras[2] - m[11]];
  const idx = [];
  for (let c = 0; c < 3; c++) {
    const v = col(c);
...
function reportStructure(ras) {
  const n = labelAt(ras);
  const s = sc.segments.find((x) => x.num === n);
  $("hover").innerHTML = s ? `<b>${s.name}</b> \xB7 ${ras.map((v) => v.toFixed(0)).join(", ")} mm` : `unlabelled \xB7 ${ras.map((v) => v.toFixed(0)).join(", ")} mm`;
}
var resize = () => {
  const dpr = Math.min(2, globalThis.devicePixelRatio || 1);
  for (const n of NAMES) {
...
var groupControls = sc.groups.map((g) => ({
  label: g.name,
  ...
  color: sc.segments.find((s) => s.num === g.segments[0])?.color ?? [0.62, 0.9, 1]
}));
...
requestAnimationFrame(() => {
  resize();
  converge(8);
  status(`${sc.segments.length} structures as surfaces \xB7 CT streaming\u2026`);
});
function readyMsg() {
  const s = sc.manifest.source;
  return `NLST ${s.patientID} \xB7 ${sc.dims[0]}\xD7${sc.dims[1]}\xD7${sc.dims[2]} \xB7 ${sc.segments.length} TotalSegmentator structures in ${sc.groups.length} groups`;
}
globalThis.colorize = {
  state: () => ({
