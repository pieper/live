// nnLive inference worker for the SlicerLive gallery demo. A persistent warm
// ORT-Web WebGPU session: create once, warm once (compiles shaders), then each
// click is a short warm forward on a P^3 patch. Protocol:
//   { type:'init', modelUrl, patch }              -> { type:'ready', loadMs, warmMs }
//   { type:'infer', input:ArrayBuffer, lo, seq }  -> { type:'result', mask:ArrayBuffer, lo, seq, runMs, fg }
// input is [1,8,P,P,P] f32 (built by SlicerLive's live-segmenter buildPatchInput);
// mask is P^3 uint8 (argmax of the 2 logit channels). Adapted from nnLive's
// livemodule/probe/interactive-worker.js.
const VER = '1.27.0';
...
self.onmessage = async (e) => {
  ...
  try {
    if (m.type === 'init') {
      ...
      const lg = out['logits'] || out[Object.keys(out)[0]], data = lg.data, D3 = P * P * P;
