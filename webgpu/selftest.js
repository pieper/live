...
var MARKUP_LISTS = [
  { name: "MarkupsRed", color: [0.95, 0.2, 0.2], radius: 5 },
  { name: "MarkupsGreen", color: [0.2, 0.85, 0.3], radius: 3.5 },
  { name: "MarkupsBlue", color: [0.2, 0.45, 0.95], radius: 7 },
  { name: "MarkupsYellow", color: [0.95, 0.85, 0.1], radius: 2.5 }
];
var POINTS_PER_LIST = 25;
function rng(seed) {
  let a = seed >>> 0;
  return () => {
    a = a + 1831565813 >>> 0;
...
async function buildVolumeAndFiducials(dev, onBytes) {
...
var SEGMENTS = [
  { name: "Brain", color: [0.9, 0.2, 0.2], test: (v) => v > 40 && v < 120 },
  { name: "High", color: [0.2, 0.8, 0.5], test: (v) => v > 120 }
];
async function buildSegmentation(dev, onBytes) {
  const sv = await loadSceneVolumeField(dev, SCENES.MRHead, onBytes);
  const v = sv.voxels;
  const lab = new Uint8Array(v.length);
  const counts = [0, 0];
  for (let i = 0; i < v.length; i++) {
    if (SEGMENTS[1].test(v[i])) {
      lab[i] = 2;
      counts[1]++;
    } else if (SEGMENTS[0].test(v[i])) {
      lab[i] = 1;
      counts[0]++;
    }
  }
...
// render/demos/selftest-browser.ts
...
async function main() {
  ...
  if (which === "fiducials") {
    ...
    center = sc.sv.center;
    radius = sc.sv.radius;
    label = `${sc.sv.name} \xB7 Brain ${sc.counts[0].toLocaleString()} + High ${sc.counts[1].toLocaleString()} voxels`;
  } else {
    const sv = await loadSceneVolumeField(gpu.device, SCENES.CTACardio, prog);
    fields = [sv.field];
    center = sv.center;
    radius = sv.radius;
    label = `${sv.name} ${sv.dims.join("\xD7")}`;
  }
  ...
  const draw = () => {
    ...
    status(`${label} \xB7 ${(performance.now() - t0).toFixed(0)} ms/frame \xB7 drag=rotate \xB7 shift/middle=pan \xB7 right=zoom`);
  };
  const resize = () => {
  ...
  if (widget) {
    globalThis.__xformDbg = {
      snapshot: () => {
        const r = canvas.getBoundingClientRect();
        return {
          handles: widget.handleList(widget.scaleFor(camera.position, focalPx())).map((h) => ({ id: h.id, world: h.world, kind: h.data.kind, axis: h.data.axis })),
