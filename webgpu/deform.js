...
async function buildDeformScene(dev, sceneUrl = "https://pieper.github.io/live/legacy/scenes/MRHead.json", onBytes) {
  ...
  let hover = null;
  const buildPins = () => {
    const pins = sources.map((c) => ({ center: c, radius: 6, color: [0.25, 0.85, 1, 0.5] }));
...
async function main() {
  ...
  const a3d = mountAdaptive3d({
    ...
    gpu,
    onFrame: () => status(`${sc.sv.name} \xB7 TPS landmark deform \xB7 gain ${sc.warp.gain.toFixed(2)} \xB7 ${msg}`)
  });
  const draw = () => a3d.draw();
